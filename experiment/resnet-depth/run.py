...
def run_model(model_name, model_dir, images, labels):
    output_dir = os.path.join(model_dir, "output", f"posit{func_suffix}")
    ...
    json_data["ground_truth_inference_time"] = []
    json_data["labels"] = []
    ...
    json_data["posit_inference_time"] = []
...
def main():
    ...
    for depth in depths:
        model_name = f"resnet{depth}-v1-7"
        model_dir = os.path.join(args.workdir, f"resnet{depth}")
        os.makedirs(model_dir, exist_ok=True)
        run_model(model_name, model_dir, images, labels)
