...
def compile_model(model_url, model_name, work_dir):
    output_dir = os.path.join(work_dir, "output", f"posit{func_suffix}")
    os.makedirs(output_dir, exist_ok=True)
    json_log_file = os.path.join(output_dir, "compile_log.json")


    model_tar_gz = os.path.join(work_dir, f"{model_name}.tar.gz")
    ...
    model_name = f"{model_name}-ground-truth"
    model_name_posit = f"{model_name}-posit{func_suffix}"
...
def main():
    ...
    for depth in depths:
        model_url = f"https://github.com/onnx/models/raw/main/validated/vision/classification/resnet/model/resnet{depth}-v1-7.tar.gz"
        model = f"resnet{depth}-v1-7"
        work_dir = os.path.join(args.workdir, f"resnet{depth}")
        os.makedirs(work_dir, exist_ok=True)
        compile_model(model_url, model, work_dir)
        print(f"Compiled {model} with posit {func_suffix} and saved to {work_dir}")
