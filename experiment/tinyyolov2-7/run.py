...
def main():
    ...
    images, labels, sizes = voc_dataloader.get_random_sample(num_samples)
    ...
    for i, image in enumerate(images):
        ...
        gt_final_outputs = sess.run(inputs)
        time_end = time.time()
        ...
        save_ref(f"ground-truth-{i}", gt_final_outputs, output_dir)
        
        json_data["labels"].append(labels[i])
        json_data["original_sizes"].append(sizes[i])
    ...
    for i, image in enumerate(images):
        ...
        for posit_out in posit_outs:
            posit_final_outputs.append(getDoubleArray[func_suffix](posit_out))

        save_ref(f"posit-{i}", posit_final_outputs, output_dir)
