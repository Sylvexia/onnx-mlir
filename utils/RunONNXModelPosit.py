...
print("Using Posit<{},{}>".format(args.n_bit, args.es))
print(f"suffix: {func_suffix}")
...
def generate_random_input(input_signature, input_shapes):
    ...
    for i, sig in enumerate(signature):
        ...
        rinput = np.random.uniform(
            lb, ub, explicit_shape).astype(random_element_type)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if args.load_so:
            ...
            command_str += ["--enable-posit",
                            f"--n-bits={args.n_bit}", f"--es-val={args.es}"]
            command_str += [f"-L{os.environ['CUSTOM_POSIT_LIB_DIR']}",
        ...
        print("Input signature: ", input_signature)
        print("Output signature: ", output_signature)
        ...
        if args.load_ref:
            ...
            inputs = read_input_from_refs(
                len(input_names), args.load_ref_from_numpy)
        else:
        ...
        for i in range(args.warmup):
            start = time.perf_counter()
            posit_outs = sess.run(posit_inputs)
            end = time.perf_counter()
        ...
        if args.verify:
            ...
            if args.verify.lower() == "onnxruntime":
                ...
                if args.load_ref:
                    ref_outs = read_output_from_refs(
                        len(output_names), args.load_ref)
                elif args.load_ref_from_numpy:
                    ref_outs = read_output_from_refs(
            ...
            if args.verify_every_value:
                for i, name in enumerate(output_names):
                    print(
                        "Verifying value of {}:{}".format(
                            name, list(outs[i].shape)),
                        "using atol={}, rtol={} ...".format(
                            args.atol, args.rtol),
                    )
