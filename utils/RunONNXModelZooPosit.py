...
VERBOSITY_LEVEL = {"debug": 10, "info": 5,
                   "warning": 1, "error": 0, "critical": 0}
...
RUN_ONNX_MODEL_CMD = [os.path.join(sys.path[0], "RunONNXModelPosit.py")]
...
def clone_modelzoo_source(repo_url, work_dir):
    ...
    # Either repo doesn't exist, or it's invalid
        ...
        if Path(repo_dir).exists():
            logger.error(
                "repo {} not a git repo, no overwrite".format(repo_dir))
            return None
...
def check_model(model_path, model_name, compile_args, report_dir):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        compile_options = "--compile-args=" + \
            (compile_args if compile_args else "-O3")
        ...
        if name not in all_model_names:
            logger.error(
                ...
                difflib.get_close_matches(
                    name, all_model_names, len(all_model_names)),
            )
            return
